...
def fetch_users_first_name(access_token):
    user_list = make_vk_api_request('users.get', access_token=access_token)['response']
    if len(user_list) == 0:
        raise VkRequestError('user_list is empty', 0)
